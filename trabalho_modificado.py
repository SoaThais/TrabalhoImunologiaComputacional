...
# Contorno
boundary = MeshFunction("size_t", mesh, 'malha_B_facet_region.xml')
...
u_n1_0 = Expression('abs((3.94 - 4.13) * x[0] + (3.23 - 3.69) * x[1] + (3.69 * 4.13 - 3.23 * 3.94)) / sqrt(pow((3.94 - 4.13), 2) + pow(3.23 - 3.69, 2)) < 0.01 or abs((3.62 - 3.94) * x[0] + (3.69 - 3.86) * x[1] + (3.86 * 3.94 - 3.69 * 3.62)) / sqrt(pow(3.62 - 3.94, 2) + pow(3.69 - 3.86, 2)) < 0.01 ? 0.2 : 0.0', element, degree = 2)
...
bc_1 = DirichletBC(U, Constant((0, 0)), boundary, 10)

bc_2 = DirichletBC(U, Constant((0, 0)), boundary, 20)

bc_3 = DirichletBC(U, Constant((0, 0)), boundary, 30)
...
vtkfile_u_1     = File('output_modificado/C_p.pvd')
vtkfile_u_2     = File('output_modificado/C_l.pvd')
vtkfile_u_3     = File('output_modificado/P.pvd')
vtkfile_u_4     = File('output_modificado/U.pvd')
vtkfile_u_solid = File('output_modificado/Phi_f.pvd')
vtkfile_w_solid = File('output_modificado/vel.pvd')
...
for i in range(num_steps):
    ...
    total_leucocytes = assemble(u_2 * dx)
    leucocyte_count.append(total_leucocytes)
...
for i in range(num_steps): 
    print(f'Número total de leucócitos no tempo {i * end_time / num_steps}: {total_leucocytes}')